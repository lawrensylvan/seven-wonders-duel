...
export const ActionHandler = (io) => {
    ...
    const processGameEvent = (tableId, event) => {
        const processor = state.getEventProcessor(tableId, event)
        processor.process()
        state.getPlayers(tableId).forEach(p => {
            const step = processor.toStep(p)
            if(step) {
                io.to(p).emit('action', {type:'gameStepPushed', tableId, step})
            }
        })
        processor.nextEvents(tableId).forEach(event => {
    ...
    return {
        ...
        move: ({tableId, move}, socket) => {
            ...
            const processor = state.getMoveProcessor(tableId, player, move)
            if(!processor.isValid()) throw 'This move is not valid'
            processor.process()
            state.getPlayers(tableId).forEach(p => {
                const step = processor.toStep(p)
                if(step) {
